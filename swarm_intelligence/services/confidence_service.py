...
class ConfidenceService:
    ...
    def record_confidence_snapshot(self, agent_id: str, value: float, reason: str):
        """Creates a new ConfidenceSnapshot for an agent."""
        query = """
    ...
    def penalize_for_override(self, agent_id: str, penalty: float = 0.1):
        """Applies a penalty for a human override and records a snapshot."""
        last_credibility = self.get_last_confidence(agent_id)
        penalized_value = last_credibility - penalty
        self.record_confidence_snapshot(agent_id, penalized_value, "human_override_penalty")

    def reinforce_for_success(self, agent_id: str, reinforcement: float = 0.05):
        """Applies a reinforcement for a successful outcome and records a snapshot."""
        last_credibility = self.get_last_confidence(agent_id)
        reinforced_value = last_credibility + reinforcement
        self.record_confidence_snapshot(agent_id, reinforced_value, "successful_outcome_reinforcement")
