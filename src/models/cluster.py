...
class AlertCluster(BaseModel):
    ...
    correlation_score: float = Field(
        ...
        ge=0.0, 
        le=1.0, 
        description="Confidence of grouping (0.0-1.0)"
    )
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Cluster formation time")
    ...
    def from_alerts(cls, alerts: list[NormalizedAlert], correlation_score: float) -> "AlertCluster":
        ...
        primary_service = max(service_counts, key=service_counts.get)
