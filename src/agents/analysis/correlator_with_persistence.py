"""
Correlator Agent com Persistência - Enterprise Grade (Produção)

Versão com resiliência, observabilidade, governança E persistência em Neo4j.
"""
...
from src.core.neo4j_persistence import Neo4jPersistence, CorrelationPattern, CorrelationEvent, PatternType

logger = logging.getLogger(__name__)


class CorrelatorAgentWithPersistence:
    """
    Agente Correlator com persistência em Neo4j.
    
    Estende CorrelatorAgentEnterprise com:
    - Armazenamento de padrões detectados
    - Histórico de correlações
    - Análise de tendências
    - Reutilização de padrões conhecidos
    """
    
    agent_id = "correlator-with-persistence"
    ...
    def __init__(
        ...
        """Inicializa agente com persistência."""
        ...
        self.known_patterns_cache: Dict[str, Dict[str, Any]] = {}
    
    def analyze(self, alert: NormalizedAlert) -> SwarmResult:
        """
        Analisa correlações com persistência.
        """
        ...
        try:
            ...
            # Carregar padrões conhecidos do serviço
            self._load_known_patterns(alert.service)
            ...
            # Persistir padrões e eventos
            self._persist_patterns_and_events(alert, decision_id)
    ...
    def _load_known_patterns(self, service_name: str) -> None:
        """Carrega padrões conhecidos do Neo4j."""
        if not self.persistence.connected:
            logger.debug("Neo4j not connected. Skipping pattern loading.")
            return
        
        try:
            patterns = self.persistence.get_patterns_by_service(service_name, limit=50)
            self.known_patterns_cache = {p.get("pattern_id"): p for p in patterns}
            
            logger.info(f"Loaded {len(patterns)} known patterns for {service_name}")
            self.observability.metrics.set_gauge(
                "known_patterns_loaded",
                float(len(patterns)),
    ...
    def _analyze_log_metric_correlation_resilient(self, alert: NormalizedAlert) -> None:
        """Analisa correlação log-métrica com resiliência e persistência."""
        try:
    ...
    def _persist_patterns_and_events(self, alert: NormalizedAlert, decision_id: str) -> None:
        """Persiste padrões e eventos em Neo4j."""
        if not self.persistence.connected:
            logger.debug("Neo4j not connected. Skipping persistence.")
            return
        
        try:
            for pattern_data in self.detected_patterns:
                pattern_id = f"{alert.service}_{pattern_data['type']}_{int(time.time())}"
                ...
                pattern = CorrelationPattern(
                    ...
                    correlation_coefficient=pattern_data.get("correlation_coefficient", 0.0),
                    p_value=pattern_data.get("p_value", 1.0),
                    lag_offset=pattern_data.get("lag_offset", 0),
                    significance=pattern_data.get("significance", "UNKNOWN"),
                    description=pattern_data.get("action", ""),
            ...
            logger.info(f"Persisted {len(self.detected_patterns)} patterns for {alert.service}")
    ...
    def get_pattern_history(self, service_name: str, days: int = 30) -> Dict[str, Any]:
        """Recupera histórico de padrões de um serviço."""
        if not self.persistence.connected:
            return {}
        
        return self.persistence.get_pattern_statistics(service_name, days)
    ...
    def get_status(self) -> dict:
        ...
        return {
            ...
            "persistence": {
                "connected": self.persistence.connected,
                "uri": self.persistence.uri
            }
        }
